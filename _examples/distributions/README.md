...
|---|---|---|
| `uniform_rand` | `uniform_rand(min, max)` | Flat distribution, every value equally likely |
| `zipf` | `zipf(s, v, max)` | Power-law skew, low values dominate |
| `norm_rand_f` | `norm_rand_f(mean, stddev, min, max, precision)` | Bell curve centered on mean |
| `exp_rand_f` | `exp_rand_f(rate, min, max, precision)` | Exponential decay from min |
| `lognorm_rand_f` | `lognorm_rand_f(mu, sigma, min, max, precision)` | Right-skewed with a long tail |
...
| `set_normal` | `set_normal(values, mean, stddev)` | Normal distribution over indices; `mean` index picked most often |
| `set_exp` | `set_exp(values, rate)` | Exponential distribution over indices; lower indices picked most often |
| `set_lognormal` | `set_lognormal(values, mu, sigma)` | Log-normal distribution over indices; right-skewed selection |
| `set_zipfian` | `set_zipfian(values, s, v)` | Zipfian distribution over indices; strong power-law skew toward first items |
