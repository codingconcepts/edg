...
func inspectSpanner(ctx context.Context, db *sql.DB, schema string) ([]Table, error) {
	...
	if err := spannerBuildDDL(ctx, db, schema, tableMap, tableOrder); err != nil {
		return nil, err
	}
	return buildResult(tableMap, tableOrder), nil
}
...
func spannerBuildDDL(ctx context.Context, db *sql.DB, schema string, tableMap map[string]*Table, order []string) error {
	for _, name := range order {
		t := tableMap[name]
		var lines []string
